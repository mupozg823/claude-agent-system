...
## 차단 목록
rm -rf, force push main, git reset --hard, DROP DB/TABLE, curl|sh, shutdown, npm publish, sudo, 민감파일(.ssh/.aws/.env)

## 자동 승인
ls, cat, git, npm, node, python, eslint, docker, gh, 모든 MCP 도구, 비민감 Write/Edit
...
```
~/.claude/
├── hooks/           16 JS 모듈 + lib/paths.js + lib/utils.js
